...
class APIWooCommerce {
    ...
    async isAWatch(id) {
        console.log("isAWatch("+id+")");
        let isAWatch = false;
        const allWatches = await this.getWatches();
        allWatches.forEach(element => {
            if (element.id == id) {
                isAWatch = true;
            }
